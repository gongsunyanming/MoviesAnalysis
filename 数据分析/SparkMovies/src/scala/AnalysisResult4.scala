...
object AnalysisResult4 {
  def main(args: Array[String]): Unit = {
    ...
    // SparkSql数据分析
    val averageScoreData = spark.sql(
      """
        |select movie_type as name, round(avg(cast(`评分` as double)), 1) as value
        |from(
        |     select element as movie_type, `评分`
        |     from movies lateral view explode(split(`电影类型`, ', ')) t as element
        |)subquery
        |group by movie_type order by value desc
        |""".stripMargin)

    averageScoreData.show()
    ...
    averageScoreData.write.mode(SaveMode.Append)
      .jdbc(s"$url/$database", "part4", connectionProperties(user, password))
  }
  // 创建链接属性对象
