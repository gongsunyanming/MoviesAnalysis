...
object AnalysisResult5 {
  def main(args: Array[String]): Unit = {
    ...
    val actorAverageScoreData = spark.sql(
      """
        |select actor as name, round(avg(cast(`评分` as double)), 1) as value
        |from(
        |     select element as actor, `评分`
        |     from movies lateral view explode(split(`主演`, ', ')) t as element
        |)subquery
        |group by actor order by value desc
        |""".stripMargin)

    actorAverageScoreData.show()
    ...
    actorAverageScoreData.write.mode(SaveMode.Append)
      .jdbc(s"$url/$database", "part5", connectionProperties(user, password))
  }
  // 创建链接属性对象
